...
@Component("read-hello-message")
@jakarta.annotation.Generated(value = "io.zenwave360.sdk.plugins.SpringCloudStreams3Plugin", date = "2024-01-04T11:19:50.961590828+01:00")
public class ReadHelloMessageConsumer implements Consumer<Message<HelloMessagePayload>> {

    protected Logger log = LoggerFactory.getLogger(getClass());

    protected IReadHelloMessageConsumerService service;
    protected StreamBridge streamBridge;
    protected Map<Class<? extends Exception>, String> errorQueueMap;

    public ReadHelloMessageConsumer(IReadHelloMessageConsumerService service, @Autowired(required=false) StreamBridge streamBridge) {
        this.service = service;
        this.streamBridge = streamBridge;
    }

    @Value("#{${spring.cloud.stream.bindings.read-hello-message-in-0.dead-letter-queue-error-map:{:}}}")
    ...
    public void accept(Message<HelloMessagePayload> message) {
        ...
        try {
            ...
            if(payload instanceof HelloMessagePayload) {
                var headers = new IReadHelloMessageConsumerService.HelloMessagePayloadHeaders();
                headers.putAll(message.getHeaders());
                service.readHelloMessage((HelloMessagePayload) payload, headers);
                return;
            }
